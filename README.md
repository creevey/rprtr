...
| `--config`         | `-c`  | auto-detect     | Playwright config path used to enable the run buttons and pre-run test listing at startup (Playwright only). When omitted, the server discovers `playwright.config.*` in the working directory; a `vitest.config.*` is discovered too, seeding Vitest run buttons. A registering reporter always overrides this. |
