...
- [ ] 2.2 Implement in `src/vitest.ts` `sendRegister`/`onInit` (D2): `configFile` from `vitest.config.configFile` with the `vitest.vite.config.configFile` fallback spike; omit when neither resolves. Verify: tests from 2.1 pass && `cd tests && bun test vitest-browser-integration.test.ts` (harness still green)
