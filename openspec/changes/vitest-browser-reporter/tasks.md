...
- [x] 6.2 Full gate + Playwright regression + focused suites. Verify: `bun run check && bun run test:playwright && cd tests && bun test vitest-helpers.test.ts vitest-reporter.test.ts vitest-browser-integration.test.ts artifact-routes.test.ts`
