...
**Split `test:bun` into a unit glob and a browser glob, rather than gating inside the test files.** A `test.skipIf(browserMissing)` would keep the files in the bare job and turn a real regression into a silent skip on any machine where browser resolution hiccups. Two scripts — `test:bun` for the runner-independent files and a browser-tests script executed by the container job — make the requirement structural: the bare job cannot run a browser test, and the container job cannot forget one. Naming the browser-dependent files by a shared suffix keeps the globs declarative.

**Assert a non-empty run inside the vitest integration helper.** `spawnFixtureVitestRun` already asserts the child's exit code; it should also assert the run reported at least one test. This is the guard that would have turned "0 passed, 0 failed" into a clear failure the first time, independent of the job layout, and it costs one assertion.
