# vitest-reporter Specification

## Purpose

Reports Vitest Browser Mode `toMatchScreenshot` results to the rprtr server and artifacts: the test event stream, image normalization from Vitest screenshot artifacts (including passing assertions), dev zero-copy and CI portable modes, offline/static parity, and register-time directory allowlisting.
